	...
	if ((resized && GetAsyncKeyState(VK_LBUTTON) >= 0) || toggle_fullscreen) 
	{
		...
		for (u32 i{ 0 }; i < _countof(_surfaces); ++i) {
			if (win.get_id() == _surfaces[i].window.get_id())
			{
				...
				{
					win.set_fullscreen(!win.is_full_screen());
					// The default window procedure will play a system notification sound when pressing the Alt+Enter keyboard combination if WM_SYSCHAR is not handled. By returning 0 we can tell the system that the we handled this message.
					return 0;
				}
				else {
...
test_initialize(){
	...
	if (!graphics::initialize(graphics::graphics_platform::opengl)) return false;
	platform::window_init_info info[]{
		...
		{&win_proc, nullptr, L"Render window 4", 250, 250, 400, 800},
	};
