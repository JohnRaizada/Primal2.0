...
/* If you get any compilation or linker errors than make sure that
 ...
 3) the library path in the following section to the correct location*/
#if _DEBUG
...
#pragma comment(lib,"C:\\Program Files\\Autodesk\\FBX\\FBX SDK\\2020.3.2\\lib\\vs2019\\x64\\debug\\zlib-md.lib")
#endif // _DEBUG
 // LNK4099 PDB not found warnings can be resolved either by installing FBX SDK PDBs (separate download) or by disabling this warning in linker options (Linker command line: /ignore:4099
namespace primal::tools
{
	...
	void
		...
		// NOTE: number of LODs is exclusively the base mesh (LOD 0)
	...
	bool
		...
		const s32 num_polyps{ fbx_mesh->GetPolygonCount() };
		if (num_polyps <= 0) return false;
		// Get vertices
		...
		for (s32 i{ 0 };i < num_indices;++i) {
			...
			// Did we encounter this vertex before? If so, just add its index. If not, add the vertex and a new index.
		...
		if (import_normals)
		{
			...
			{
				// Something went wrong with importing normals from FBX. Fall back to our own normal calculation method.
		...
		if (import_tangents)
		{
			...
			{
				// Something went wrong with importing tangents from FBX. Fall back to our own tangent calculation method.
	...
	EDITOR_INTERFACE void
		...
		assert(file && data);
		scene scene{};
		// NOTE: anything that involves using the FBX SDK should be single-threader
		{
