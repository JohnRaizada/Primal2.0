...
namespace primal::graphics::d3d12::core
{
	namespace {
		...
		public:
			...
			explicit d3d12_command(id3d12_device *const device, D3D12_COMMAND_LIST_TYPE type) {
				...
				for (u32 i{ 0 }; i < frame_buffer_count; ++i) {
					...
					if (FAILED(hr)) goto _error;
					NAME_D3D12_OBJECT_INDEXED(frame.cmd_allocator, i, type == D3D12_COMMAND_LIST_TYPE_DIRECT ? L"GFX Command List" : type == D3D12_COMMAND_LIST_TYPE_COMPUTE ? L"Compute Command List" : L"Command List");
				}
		...
		private:
			struct command_frame
			{
				...
				void wait(HANDLE fence_event, ID3D12Fence1* fence) {
					...
					// If the current fence value is still less than "fence_value" then we know the GPU has not finished excuting the command lists since it has not reached the "_cmd_queue->Signal()" command
					if (fence->GetCompletedValue() < fence_value)
					{
						// We have the fence create an event which is signaled when the fence's current value equals "fence_value"
						DXCall(fence->SetEventOnCompletion(fence_value, fence_event));
						// Wait until the fence has triggered the event that its current value has reached "fence_value" indicating that command queue has finished executing.
		...
		// NOTE: this function can be expande in functionality with, for example, checking if any output devices (i.e. screens) are attached, enumerate the supported resolutions, provide a means for the user to choose which adapter to use in a multi-adapter setting, etc.
		IDXGIAdapter4*
		...
		void __declspec(noinline)
			...
			// NOTE: we clear this flag in the beginning. If we'd clear in at the end then it might overwrite some other thread that was trying to set it. It's fine if overwriting happens before processing the items.
...
#pragma message("WARNING: GPU based validation is enabled. This will considerably slow down the renderer!")
	...
	void
		shutdown() {
		gfx_command.release();
		// NOTE: we don't call process_deferred_releases at the end because some resources (such as swap chains) can't be released before their depending resources are released.
		for (u32 i{ 0 }; i < frame_buffer_count; ++i) {
		...
		// NOTE: some modules free their descriptors when they shutdown. We process those by calling process_deferred_free once more.
		...
		// NOTE: some types only use deferred release for their resources during shutdown/reset/clear. To finally release these resources we call process_deferred_releases once more.
	...
	void 
		set_deferred_released_flag() { deferred_releases_flag[current_frame_index()] = 1; }
	surface
	...
	void
		...
		// Wait for the GPU to finish with the command allocator and reset the allocator once the GPU is done with it. This frees the memory that was used to store commands.
		...
		cmd_list->RSSetScissorRects(1, &surface.sciccor_rect());
		// Depth prepass
		...
		// Done recording commands. Now execute command, signal and increment the fence value for next frame.
