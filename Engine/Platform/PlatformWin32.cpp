...
namespace primal::platform {
	namespace {
		...
		void 
			...
			{
				// NOTE: we also resize while in fullscreen mode to support the case when the user changes the screen resolution
		...
		void
			...
			if (info.is_fullscreen != is_fullscreen)
			{
				...
				if (is_fullscreen)
				{
					// Store the current window dimensions so they can be restarted when switching out of fullscreen state.
					GetClientRect(info.hwnd, &info.client_area);
					RECT rect;
