...
namespace primal::tools {
	namespace {
		...
		void
			...
			const f32 cos_alpha{ XMScalarCos(pi - smoothing_angle * pi / 100.f) };
			const bool is_hard_edge{ XMScalarNearEqual(smoothing_angle, 100.f, epsilon) };
				...
				for (u32 j{ 0 }; j < num_refs; ++j)
				{
					...
					if (!is_hard_edge)
					{
						for (u32 k{ j + 1 }; k < num_refs; ++k)
						{
							...
							if (!is_soft_edge)
							{
								// NOTE: we're accounting for the length of n1 in this calculation because it can possibly change in this loop iteration. We assume unit length for n2. cos(angle) = dot(n1,n2) / (||n1||*||n2||)
		...
		void
			...
			for (auto& lod : scene.lod_groups) {
				...
				for (auto& m : lod.meshes) {
					// Id more than one material is used in this mesh then split it into submeshes.
